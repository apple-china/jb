...
public class AuthController {
  ...
  record PasswordLogin(@Schema(title="账号",description="去除首尾空格后仅允许字母和数字，长度为 6–12 位。",example="operator01",minLength=6,maxLength=12,pattern="^[A-Za-z0-9]{6,12}$") @NotBlank String username,@Schema(title="密码",description="密码原文，长度为 6–12 位。",example="******",minLength=6,maxLength=12) @NotBlank @Size(min=6,max=12) String password){}
  @Schema(name="PasswordChangeRequest",title="修改密码请求",description="首次登录或主动修改密码使用。")
  record PasswordChange(@Schema(title="当前密码",description="当前有效密码。",example="******") @NotBlank String currentPassword,@Schema(title="新密码",description="长度为 6–12 位。",example="newpass",minLength=6,maxLength=12) @NotBlank @Size(min=6,max=12) String newPassword){}
