...
public class SuperAdminBootstrap implements ApplicationRunner {
  ...
  @Override public void run(ApplicationArguments args){Integer count=jdbc.queryForObject("SELECT count(*) FROM app_user WHERE role='SUPER_ADMIN'",Integer.class);if(count!=null&&count>0)return;if(username.isBlank()||password.isBlank()||dingTalkUserId.isBlank())throw new IllegalStateException("首次生产启动必须配置唯一超管账号、密码和钉钉ID");SessionService.validatePassword(password);jdbc.update("INSERT INTO app_user(id,username,password_hash,dingtalk_user_id,dingtalk_username,nickname,role,is_active,is_attending,can_modify_appointments,can_cancel_appointments,must_change_password) VALUES (?,?,?,?,?,?,'SUPER_ADMIN',true,true,true,true,true)",UUID.randomUUID(),username.trim(),passwords.encode(password),dingTalkUserId.trim(),nickname.trim(),nickname.trim());}
}
