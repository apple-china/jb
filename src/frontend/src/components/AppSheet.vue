  ...
  <Teleport to="body">
    <div v-if="open" class="sheet-backdrop" @click.self="$emit('close')">
      <section class="sheet" :class="{ fullscreen }" role="dialog" aria-modal="true" :aria-label="title">
        <header><div class="sheet-heading"><h2>{{ title }}</h2><slot name="subtitle"><small v-if="subtitle">{{ subtitle }}</small></slot></div><button class="icon-button" aria-label="关闭" @click="$emit('close')"><X :size="20" /></button></header>
