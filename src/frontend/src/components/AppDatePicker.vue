...
const root=ref<HTMLElement|null>(null),open=ref(false),view=ref(new Date())
...
function show(){view.value=props.modelValue?parse(props.modelValue):new Date();open.value=true;nextTick(()=>root.value?.querySelector<HTMLElement>('.calendar-day.selected')?.focus())}
function move(months:number){view.value=new Date(view.value.getFullYear(),view.value.getMonth()+months,1)}
function select(value:string){emit('update:modelValue',value);open.value=false}
...
<template>
  <div ref="root" class="date-picker">
    ...
    <div v-if="open" class="calendar-popover" role="dialog" :aria-label="`${label}日历`">
      ...
      <div class="calendar-grid"><button v-for="item in days" :key="item.value" type="button" class="calendar-day" :class="{outside:item.outside,today:item.value===today(),selected:item.value===modelValue}" :aria-label="item.value" :aria-selected="item.value===modelValue" @click="select(item.value)">{{ item.day }}</button></div>
    </div>
  </div>
</template>
