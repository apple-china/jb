...
describe('TimeWheel',()=>{
  ...
  it('keeps unavailable minutes disabled for the selected hour',async()=>{const wrapper=mount(TimeWheel,{props:{modelValue:'12:50',slots}});await wrapper.vm.$nextTick();expect(wrapper.find('[aria-label="分钟"] [data-value="10"]').attributes('disabled')).toBeDefined();await wrapper.find('[aria-label="小时"] [data-value="13"]').trigger('click');await wrapper.find('[aria-label="分钟"] [data-value="00"]').trigger('click');expect(wrapper.emitted('update:modelValue')?.at(-1)).toEqual(['13:00'])})
