...
interface WheelItem { key:string;value:string;available:boolean;reason?:string }
...
const minuteItems=computed<WheelItem[]>(()=>minutes.value.map(minute=>{const slot=slotMap.value.get(`${selectedHour.value}:${minute}`);return {key:minute,value:minute,available:!!slot?.available,reason:slot?.reason}}))
...
<template>
  ...
  <div ref="minuteWheel" class="time-wheel" role="listbox" aria-label="分钟" @scroll.passive="settle('minute')">
      <button v-for="item in minuteItems" :key="item.key" type="button" role="option" :data-value="item.value" :aria-selected="item.value===selectedMinute" :disabled="!item.available" :title="item.reason" :class="{selected:item.value===selectedMinute}" @click="chooseMinute(item.value)">{{ item.value }}</button>
    </div>
  </div>
</template>
