...
export interface AppointmentOperation {type:'CREATE'|'MODIFY'|'CANCEL';actorName?:string;createdAt:string;reason?:string;changes:Array<{field:string;before?:string;after?:string}>}
export interface Appointment { id:string;bookingNumber?:string;bookingDate:string;startTime:string;streamerName:string;makeupArtistId:string;makeupArtistName:string;teamId:string;teamName:string;status:AppointmentStatus;attendanceStatus:AttendanceStatus;attendanceFrozen?:boolean;attendanceEvidenceAt?:string;createdAt?:string;cancelledAt?:string;hasModification?:boolean;operations?:AppointmentOperation[];conflictOverride?:boolean;version:number;changed?:boolean;selfOperationAllowed?:boolean;resourceModificationAllowed?:boolean;timeModificationAllowed?:boolean }
