...
async function dingTalkLogin(){if(loading.value)return;loading.value='dingtalk';try{const user=await authenticateDingTalk();showAuthMessage(requested()?'已登录':`已登录，${user.nickname}`,'success');await router.replace(destinationFor(user,requested()))}catch(e){if(e instanceof ApiError&&['ACCOUNT_DISABLED','ACCOUNT_UNREGISTERED','FORBIDDEN'].includes(e.code))await router.replace({path:'/forbidden',query:{reason:e.code}});else showToast('免登异常，请联系管理员','error',5000)}finally{loading.value=''}}
