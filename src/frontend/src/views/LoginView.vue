...
async function dingTalkLogin(){if(loading.value)return;loading.value='dingtalk';try{const user=await authenticateDingTalk();showAuthMessage(requested()?'已登录':`已登录，${user.nickname}`,'success');await router.replace(destinationFor(user,requested()))}catch(e){const clientError=e as {code?:string};if(clientError.code==='USER_NOT_AUTHORIZED')await router.replace('/forbidden');else showToast('免登异常，请联系管理员','error',5000)}finally{loading.value=''}}
...
<template><main class="login-page"><section class="login-card"><BrandMark hide-subtitle/><template v-if="forcedUser"><div class="login-copy password-change-copy"><h1>请先修改密码</h1><p>请设置新密码后继续使用。</p></div><form class="password-login" autocomplete="off" data-lpignore="true" @submit.prevent="finishPasswordChange"><PasswordInput v-model="newPassword" name="access-code-new" autocomplete="off" placeholder="新密码（6–12位）" :maxlength="12"/><PasswordInput v-model="confirmPassword" name="access-code-confirm" autocomplete="off" placeholder="再次输入新密码" :maxlength="12"/><button type="submit" class="button primary full" :disabled="!changedPasswordReady||!!loading">确认修改并进入</button></form></template><template v-else><div v-if="mockLoginEnabled" class="login-copy"><span class="eyebrow">本地 Mock</span><h1>选择测试身份</h1><p>五种固定角色均由服务端校验。</p></div><div v-if="mockLoginEnabled" class="account-list"><button v-for="account in accounts" :key="account.id" type="button" :disabled="!!loading" @click="login(account.id)"><span class="account-icon"><component :is="account.icon" :size="21"/></span><span><strong>{{ account.name }}</strong><small>{{ account.note }}</small></span><span class="account-enter">{{ loading===account.id?'进入中…':'进入' }}</span></button></div><form class="password-login" autocomplete="off" data-lpignore="true" @submit.prevent="passwordLogin"><h2>账号登录</h2><input v-model="username" name="account" autocomplete="off" data-lpignore="true" data-1p-ignore data-bwignore="true" autocapitalize="off" :spellcheck="false" inputmode="text" placeholder="账号"/><PasswordInput v-model="password" name="access-code" autocomplete="off" placeholder="密码" :maxlength="12"/><button type="submit" class="button primary login-submit full" :disabled="!loginReady||!!loading">{{ loading==='password'?'登录中…':'登录' }}</button><button type="button" class="button dingtalk-login full" :disabled="!!loading" @click="dingTalkLogin">{{ loading==='dingtalk'?'免登中…':'钉钉免登' }}</button></form></template><div v-if="mockLoginEnabled" class="login-foot"><UsersRound :size="16"/> 固定测试数据，不包含真实个人信息</div></section><AppToast :message="toast.message" :kind="toast.kind" /></main></template>
