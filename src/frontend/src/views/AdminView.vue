...
<template>
  <main class="admin-app">
    ...
    <AppSheet :open="!!detail && !action" title="预约详情" :subtitle="detailSubtitle" @close="detail = null"><div v-if="detail" class="detail-stack detail-sections">
      ...
      <div v-if="detail.status === 'ACTIVE' && ((canModify && detail.attendanceStatus !== 'LATE') || canCancel)" class="action-row"><button v-if="canModify && detail.attendanceStatus !== 'LATE'" class="appointment-action modify" @click="openAction('modify', detail)">修改</button><button v-if="canCancel" class="appointment-action cancel" @click="openAction('cancel', detail)"><X :size="17" />取消</button></div>
    </div></AppSheet>
